...
export function TaiTabs({ tabs, value, defaultValue, onValueChange, className }: TaiTabsProps) {
  ...
  return (
    <div className={cn("min-w-0", className)}>
      <div role="tablist" aria-label="Tabs" className="flex min-w-0 gap-1 overflow-x-auto border-b border-white/[0.12]">
        {tabs.map((tab, index) => {
          const selected = tab.value === activeValue;
          return <button key={tab.value} id={`tab-${tab.value}`} type="button" role="tab" aria-selected={selected} aria-controls={`panel-${tab.value}`} tabIndex={selected ? 0 : -1} disabled={tab.disabled} onClick={() => selectTab(tab.value)} onKeyDown={(event) => { if (event.key === "ArrowRight" || event.key === "ArrowDown") { event.preventDefault(); moveTab(1); } if (event.key === "ArrowLeft" || event.key === "ArrowUp") { event.preventDefault(); moveTab(-1); } if (event.key === "Home") { event.preventDefault(); focusTab(tabs.find((item) => !item.disabled)?.value ?? ""); } if (event.key === "End") { event.preventDefault(); focusTab([...tabs].reverse().find((item) => !item.disabled)?.value ?? ""); } }} className={cn("shrink-0 border-b-2 border-transparent px-3 py-3 font-mono text-[10px] uppercase tracking-widest text-zinc-500 transition-colors hover:text-white focus-visible:outline-2 focus-visible:outline-emerald-400 disabled:cursor-not-allowed disabled:opacity-40", selected && "border-emerald-400 text-white")}>
            {tab.label}
          </button>;
        })}
      </div>
      {tabs.map((tab, index) => tab.value === activeValue && <div key={tab.value} id={`panel-${tab.value}`} role="tabpanel" aria-labelledby={`tab-${tab.value}`} tabIndex={0} className="pt-5 text-sm leading-7 text-zinc-300">{tab.content}</div>)}
    </div>
  );
}
