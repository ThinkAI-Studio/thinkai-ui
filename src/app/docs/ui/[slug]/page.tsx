...
export function generateStaticParams() {
  return catalogItems.filter((item) => item.kind === "ui").map((item) => ({ slug: item.slug }));
}
...
export default async function UiComponentPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const item = getCatalogItem(slug);
  if (!item || item.kind !== "ui") notFound();
