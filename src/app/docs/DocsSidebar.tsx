...
export function DocsSidebar({ activeSlug, activeHref, onNavigate }: { activeSlug?: string; activeHref?: string; onNavigate?: () => void }) {
    ...
    {docsGroups.map((group) => {
      ...
      return <section key={group.label} className="mb-8">
        ...
        <nav className="ml-3 border-l border-tai-border" aria-label={`${group.label} pages`}>
          {"pages" in group && group.pages.map((slug) => { const item = docPages.find((entry) => entry.slug === slug); if (!item) return null; const active = activeSlug === slug; return <Link key={slug} href={`/docs/${slug}`} onClick={onNavigate} aria-current={active ? "page" : undefined} className={`group relative block min-h-11 min-w-0 px-5 py-2.5 text-sm transition-[border-color,color,transform] hover:translate-x-1 ${active ? "-ml-px border-l border-tai-accent text-tai-text" : "border-l border-transparent text-tai-muted hover:border-tai-border-strong hover:text-tai-text"}`}><span className="block truncate">{item.title}</span></Link>; })}
          {"links" in group && group.links.map((link) => { const active = activeHref === link.href; return <Link key={link.href} href={link.href} onClick={onNavigate} aria-current={active ? "page" : undefined} className={`group relative block min-h-11 min-w-0 px-5 py-2.5 text-sm transition-[border-color,color,transform] hover:translate-x-1 ${active ? "-ml-px border-l border-tai-accent text-tai-text" : "border-l border-transparent text-tai-muted hover:border-tai-border-strong hover:text-tai-text"}`}><span className="block truncate">{link.label}</span></Link>; })}
        </nav>
      </section>;
    })}
  </aside>;
